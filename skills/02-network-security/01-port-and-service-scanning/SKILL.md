...
difficulty: beginner
tags: [network, recon, nmap, scanning, enumeration]
...
```bash
# quick top-ports TCP with versions
nmap -sV --top-ports 1000 target -oA quick

# full TCP, service + default scripts, reasonable speed
nmap -sS -sV -sC -p- -T4 target -oA full

# just "what's open", nothing else
nmap -p- --open -T4 target

# service versions only on known-open ports
nmap -sV -p 80,443,8080,8443 target

# naabu (fast) feeding nmap (detail)
