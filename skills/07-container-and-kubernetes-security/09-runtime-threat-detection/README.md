<!-- GENERATED — do not edit -->
<div align="center">
  <img src="../../../assets/brand/jihedailabs-logo.svg" alt="JihedAiLabs" width="120"/>
</div>

# Runtime Threat Detection

<div align="center">

**A <a href="https://github.com/jihedbfr-art">JihedAiLabs</a> project** — Use when detecting malicious behaviour in running containers — the runtime monitoring that catches escapes, crypto-mining, and anomalous activity that build-time controls can't.

<a href="./README.fr.md">Version française</a>

</div>

---

## Description
Use when detecting malicious behaviour in running containers — the runtime monitoring that catches escapes, crypto-mining, and anomalous activity that build-time controls can't.

## Domain & Metadata
- **Domain:** `07-container-and-kubernetes-security`
- **Maturity:** `stable`
- **Audience:** `["backend-engineer", "security-engineer", "coding-agent"]`
- **Requirements:** `["bash", "git"]`
- **Last Updated:** 2026-08-08

## Agent Instructions
The canonical agent-executable specification is available in [SKILL.md](./SKILL.md).
