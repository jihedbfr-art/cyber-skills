<!-- GENERATED — do not edit -->
<div align="center">
  <img src="../../../assets/brand/jihedailabs-logo.svg" alt="JihedAiLabs" width="120"/>
</div>

# Mapping To Attack

<div align="center">

**A <a href="https://github.com/jihedbfr-art">JihedAiLabs</a> project** — Use when tying detections to the MITRE ATT&CK framework — mapping each rule to a technique so you can measure coverage, find gaps, and prioritise what to build next.

<a href="./README.fr.md">Version française</a>

</div>

---

## Description
Use when tying detections to the MITRE ATT&CK framework — mapping each rule to a technique so you can measure coverage, find gaps, and prioritise what to build next.

## Domain & Metadata
- **Domain:** `18-detection-engineering`
- **Maturity:** `stable`
- **Audience:** `["backend-engineer", "security-engineer", "coding-agent"]`
- **Requirements:** `["bash", "git"]`
- **Last Updated:** 2026-08-08

## Agent Instructions
The canonical agent-executable specification is available in [SKILL.md](./SKILL.md).
