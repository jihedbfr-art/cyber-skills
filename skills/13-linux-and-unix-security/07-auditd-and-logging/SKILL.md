...
tags: [linux, auditd, logging, detection, forensics]
tools: [auditd, auditctl, ausearch]
---
...
```bash
# rules: /etc/audit/rules.d/*.rules  (loaded via augenrules)
# key high-value rules (examples)
...
# read the logs (learn this BEFORE an incident)
...
# protect the evidence: forward off-host to SIEM promptly (attacker can wipe local)
```
