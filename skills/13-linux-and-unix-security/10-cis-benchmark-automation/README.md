<!-- GENERATED — do not edit -->
<div align="center">
  <img src="../../../assets/brand/jihedailabs-logo.svg" alt="JihedAiLabs" width="120"/>
</div>

# Cis Benchmark Automation

<div align="center">

**A <a href="https://github.com/jihedbfr-art">JihedAiLabs</a> project** — Use when applying and verifying a Linux hardening baseline at scale — automating CIS Benchmark checks and remediation so hosts start hardened and don't drift.

<a href="./README.fr.md">Version française</a>

</div>

---

## Description
Use when applying and verifying a Linux hardening baseline at scale — automating CIS Benchmark checks and remediation so hosts start hardened and don't drift.

## Domain & Metadata
- **Domain:** `13-linux-and-unix-security`
- **Maturity:** `stable`
- **Audience:** `["backend-engineer", "security-engineer", "coding-agent"]`
- **Requirements:** `["bash", "git"]`
- **Last Updated:** 2026-08-08

## Agent Instructions
The canonical agent-executable specification is available in [SKILL.md](./SKILL.md).
