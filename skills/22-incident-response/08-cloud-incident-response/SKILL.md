...
difficulty: advanced
tags: [incident-response, cloud, aws, forensics, identity]
tools: [aws-cli]
---
...
# 3. contain identity (revoke SESSIONS, not just the key)
aws iam update-access-key --access-key-id AKIA... --status Inactive
# + attach a deny-before-timestamp policy to kill active assumed sessions

# 4. preserve a resource without destroying it
aws ec2 create-snapshot --volume-id vol-...          # snapshot first
# then isolate (quarantine SG, no egress) — do NOT terminate
```
