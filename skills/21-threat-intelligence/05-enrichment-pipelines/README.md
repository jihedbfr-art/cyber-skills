<!-- GENERATED — do not edit -->
<div align="center">
  <img src="../../../assets/brand/jihedailabs-logo.svg" alt="JihedAiLabs" width="120"/>
</div>

# Enrichment Pipelines

<div align="center">

**A <a href="https://github.com/jihedbfr-art">JihedAiLabs</a> project** — Use when building automated enrichment for indicators — adding context (reputation, WHOIS, relationships, geolocation) to raw IoCs so they become actionable intelligence.

<a href="./README.fr.md">Version française</a>

</div>

---

## Description
Use when building automated enrichment for indicators — adding context (reputation, WHOIS, relationships, geolocation) to raw IoCs so they become actionable intelligence.

## Domain & Metadata
- **Domain:** `21-threat-intelligence`
- **Maturity:** `stable`
- **Audience:** `["backend-engineer", "security-engineer", "coding-agent"]`
- **Requirements:** `["bash", "git"]`
- **Last Updated:** 2026-08-08

## Agent Instructions
The canonical agent-executable specification is available in [SKILL.md](./SKILL.md).
