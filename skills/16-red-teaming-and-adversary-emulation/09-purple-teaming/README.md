<!-- GENERATED — do not edit -->
<div align="center">
  <img src="../../../assets/brand/jihedailabs-logo.svg" alt="JihedAiLabs" width="120"/>
</div>

# Purple Teaming

<div align="center">

**A <a href="https://github.com/jihedbfr-art">JihedAiLabs</a> project** — Use when running red and blue collaboratively — red executes techniques while blue watches and tunes detection in real time, so the engagement directly improves defensive coverage.

<a href="./README.fr.md">Version française</a>

</div>

---

## Description
Use when running red and blue collaboratively — red executes techniques while blue watches and tunes detection in real time, so the engagement directly improves defensive coverage.

## Domain & Metadata
- **Domain:** `16-red-teaming-and-adversary-emulation`
- **Maturity:** `stable`
- **Audience:** `["backend-engineer", "security-engineer", "coding-agent"]`
- **Requirements:** `["bash", "git"]`
- **Last Updated:** 2026-08-08

## Agent Instructions
The canonical agent-executable specification is available in [SKILL.md](./SKILL.md).
