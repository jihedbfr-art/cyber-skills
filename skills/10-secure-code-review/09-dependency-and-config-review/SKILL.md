...
tags: [code-review, dependencies, configuration, hardening, defaults]
tools: [ripgrep, osv-scanner, dependabot]
---
...
```bash
# debug / dev settings that shouldn't ship
...
# unpinned deps (spot-check the manifests)
rg -n '"\^|"~|:latest|version: *"\*"'
```
