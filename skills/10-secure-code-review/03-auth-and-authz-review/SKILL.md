...
difficulty: advanced
tags: [code-review, authorization, authentication, access-control, idor]
tools: [grep, ripgrep]
---
...
```bash
# fetches by id — each needs an ownership/tenant check nearby
rg -n 'findById|getById|\.get\(id|WHERE id ?=|/:id|\{id\}'
# identity read from the request (suspect for authz)
rg -n 'req\.body\.(user|role)|getParameter\("(userId|role)"|headers\[.?x-user'
# where the checks live (or don't)
