...
difficulty: advanced
tags: [code-review, cryptography, hashing, encryption, randomness]
...
```bash
# weak hashes (context matters — a checksum is fine, a password hash is not)
rg -n 'MD5|SHA-?1|MessageDigest\.getInstance\("(MD5|SHA-1|SHA-256)"'
# dangerous modes / missing AEAD
rg -n 'AES/ECB|Cipher\.getInstance\("AES"\)|DES|RC4|/CBC/'
# weak randomness for security use
