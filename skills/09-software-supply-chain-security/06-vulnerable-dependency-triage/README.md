<!-- GENERATED — do not edit -->
<div align="center">
  <img src="../../../assets/brand/jihedailabs-logo.svg" alt="JihedAiLabs" width="120"/>
</div>

# Vulnerable Dependency Triage

<div align="center">

**A <a href="https://github.com/jihedbfr-art">JihedAiLabs</a> project** — Use when triaging flagged vulnerable dependencies — telling exploitable from merely-present so you fix what actually matters instead of chasing every CVE in the dependency tree.

<a href="./README.fr.md">Version française</a>

</div>

---

## Description
Use when triaging flagged vulnerable dependencies — telling exploitable from merely-present so you fix what actually matters instead of chasing every CVE in the dependency tree.

## Domain & Metadata
- **Domain:** `09-software-supply-chain-security`
- **Maturity:** `stable`
- **Audience:** `["backend-engineer", "security-engineer", "coding-agent"]`
- **Requirements:** `["bash", "git"]`
- **Last Updated:** 2026-08-08

## Agent Instructions
The canonical agent-executable specification is available in [SKILL.md](./SKILL.md).
