<!-- GENERATED — do not edit -->
<div align="center">
  <img src="../../../assets/brand/jihedailabs-logo.svg" alt="JihedAiLabs" width="120"/>
</div>

# Sso Integration Review

<div align="center">

**A <a href="https://github.com/jihedbfr-art">JihedAiLabs</a> project** — Use when reviewing a single sign-on integration between an app and an identity provider — the trust-boundary mistakes that let an attacker impersonate users or bypass the IdP.

<a href="./README.fr.md">Version française</a>

</div>

---

## Description
Use when reviewing a single sign-on integration between an app and an identity provider — the trust-boundary mistakes that let an attacker impersonate users or bypass the IdP.

## Domain & Metadata
- **Domain:** `11-identity-and-access-management`
- **Maturity:** `stable`
- **Audience:** `["backend-engineer", "security-engineer", "coding-agent"]`
- **Requirements:** `["bash", "git"]`
- **Last Updated:** 2026-08-08

## Agent Instructions
The canonical agent-executable specification is available in [SKILL.md](./SKILL.md).
