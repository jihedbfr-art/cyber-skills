<!-- GENERATED — do not edit -->
<div align="center">
  <img src="../../../assets/brand/jihedailabs-logo.svg" alt="JihedAiLabs" width="120"/>
</div>

# Passwordless And Passkeys

<div align="center">

**A <a href="https://github.com/jihedbfr-art">JihedAiLabs</a> project** — Use when implementing passwordless authentication with WebAuthn/passkeys — the phishing-resistant model, how it works, and the implementation details that make or break it.

<a href="./README.fr.md">Version française</a>

</div>

---

## Description
Use when implementing passwordless authentication with WebAuthn/passkeys — the phishing-resistant model, how it works, and the implementation details that make or break it.

## Domain & Metadata
- **Domain:** `11-identity-and-access-management`
- **Maturity:** `stable`
- **Audience:** `["backend-engineer", "security-engineer", "coding-agent"]`
- **Requirements:** `["bash", "git"]`
- **Last Updated:** 2026-08-08

## Agent Instructions
The canonical agent-executable specification is available in [SKILL.md](./SKILL.md).
